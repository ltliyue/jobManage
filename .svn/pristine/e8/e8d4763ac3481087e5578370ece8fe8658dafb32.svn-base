...
function checkDetail(id) {
	window.location.href="<%=path%>/requirement/editType?id=" + id;
}

function addNext(){
	 var url = "${webRoot}/rcode/saveInfoType?name="+$("#name").val();
	 $.post(url,function(data){
		if(data=="01"){
			window.location.href="<%=path%>/rcode/listContentType";
		}else{
...
function update(i){
	var id1 = $("#id_"+i).attr("value");
	 var url = "${webRoot}/rcode/updateInfoType?id="+id1+"&name="+$("#name").val();
	 $.post(url,function(data){
		if(data=="01"){
			window.location.href="<%=path%>/rcode/listContentType";
		}else{
...
function openDelModel(id){
	...
	var url = "${webRoot}/rcode/deletebyidType?id="+id1;
	$.post(url,function(data){
		if(data=="1"){
			window.location.href="<%=path%>/rcode/listContentType";
		} else {
