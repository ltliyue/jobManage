...
<script type="text/javascript">
				...
				function() {
					...
					if($("#loopParseRuleForMultiDiv").attr('style')==null || $("#loopParseRuleForMultiDiv").attr('style')==""){
						...
						for (var i = 0; i < tableObj.rows.length; i++) { //遍历Table的所有Row
							...
							for(var j=0; j<rowElement.cells.length-1;j++){
								if(rowElement.cells[j].firstChild.id!=null){
					...
					if($("#loopParseRuleForOneDiv").attr('style')==null || $("#loopParseRuleForOneDiv").attr('style')==""){//显示
						...
						if($("#loopVarType").val()=='0'){//数字
				    		...
				    		mvalue=mvalue+$("#loopStrList").val()+"|"+$("#loopStrEncode").val();
				    	}
	...
	function submitForm() {
		var url = '${webRoot}/parseRuleTemp/addParseRule';
								...
								if($("#urlWithLoop").val()=="1"){
										...
										if($("#loopVar").val()!=null && $("#loopVar").val()!="" && $("#loopVar").val().indexOf(",")<0){
											...
											if(varRuleList.length==0){
												...
												$.ajax({
													...
													dataType:'json',
													async:false,
													data:{'loopVarRules':varRuleList,'taskId':"${taskId}",
										...
										if($("#loopVar").val()!=null && $("#loopVar").val()!="" && $("#loopVar").val().indexOf(",")>=0){
											...
											for (i = 0; i < tableObj.rows.length; i++) { //遍历Table的所有Row
												...
												var rowElement = tableObj.rows[i];
												var varRuleList = new Array();
												for(j = 0; j < rowElement.cells.length-1; j++){
	...
	function addMidMap() {
		...
		return [input1Element,input2Element,selectElement];
	}
		...
		<div class="container-fluid search_select select-result"
			...
			<form  id='formn' >
				...
				<div class="form-group" id="scopeRegexDiv" style="display: none;">
					<label for="scopeRegex" class="content_zjnrcqlabel">外围的正则表达式内容</label> <input
