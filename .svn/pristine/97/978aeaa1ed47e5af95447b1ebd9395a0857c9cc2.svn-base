...
<script type="text/javascript">
	$(document).ready(function(){
		var url = '<%=path %>/home/usernameq?username=${username}';
		$.post(url,function(data){
			json = eval('('+data+')');
			$("#name").val(json.name);
			$("#email").val(json.email);
			$("#username").val(json.username);
			$("#idh").attr("value",json.id);
			if(json.enable==1){
	...
	function submit(){
		...
		if(status==true){
			...
			$.post(url,data1,function(data){
				if(data=="1"){
					alert("修改成功！");
					window.location.href="<%=path %>/home/base";
				}else if(data=="2"){
					alert("原始密码错误！");
				}else{
					alert("修改失败!");
...
<body style="margin:0; padding:0">
	...
	<!--内容开始-->
		...
		<form action="<%=path %>/datauser/useru" id='formn' method="post">
			<input name = "id" id="idh" type="hidden" value="">
			<div class="form-group">
				<label class="content_zjnrcqlabel">姓名</label> <input type="text"
					class="form-control content_zjnrcqtext" name='name' id="name" placeholder="姓名" datatype="*" nullmsg="请输入姓名！" errormsg=" ">
			</div>
			<div class="form-group">
			...
			<div class="form-group">
				<label for="exampleInputPassword1">确认密码</label> <input readonly="readonly" recheck="password"  errormsg="两次密码不一样！"
					type="password"  class="form-control content_zjnrcqtext" id="configpw"
					placeholder="确认密码">
			</div>
		</form>
