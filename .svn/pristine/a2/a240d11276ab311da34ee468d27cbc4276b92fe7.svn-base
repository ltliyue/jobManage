...
function hivec(v){
	if(v=='s'){
		...
		var url = "<%=path%>/datasource/hiveshows?bm="+$("#bmoc").val()+"&type=main";
		$.post(url,function(data){
			json = eval('(' + data + ')');
			var y = json.ybiao;
			var p = json.pbiao;
	...
	if(v=='h'){
		...
		var url = "<%=path%>/datasource/hdfsm?n="+$("#hbiao").val()+"&w="+$("#wbiao").val()+"&bm="+$("#bmoc").val();
		$.post(url,function(data){
			json = eval('(' + data + ')');
			alert(json);
			$("#h").removeAttr("disabled");
	...
	function tjinst(){
		...
		$("#chinst1").text(r);
		$("#chinst2").text(r);
		$("#wbiao").append("  and cf['INSTANCE_ID'] = '"+r+"'");
		$("#close7").click();
	}
</script>
