...
outline-style: dotted;
	...
	function see(id){
		window.location.href="<%=path%>/datasource/mainM?id="+id; 
	};
	...
	function clean(taskId){
		if(confirm("确定清除?")){
		var url = "<%=path%>/datasource/hdfsClean?taskId="+taskId;
...
<h2 class="content_title">${title }</h2>
			<div  class="renwu_caozuo" style="margin-right:50px">
				<a href="#" class='renwu_select' onclick="clean('')">清除所有hdfs
				</a>
			</div>
</div>
					...
					<c:if test="${not empty tasks}">
						<c:forEach var="task" items="${tasks }" varStatus="ts">
							<tr align="left" style="cursor: pointer;">
								...
								<td><a href="#" onclick="clean(${task.taskId })" class="content_xuqiumcan" >清除hdfs</a>
								</td>
							</tr>
						</c:forEach>
					</c:if>
