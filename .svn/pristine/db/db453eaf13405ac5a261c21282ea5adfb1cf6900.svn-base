...
function checkDetail(id) {
	window.location.href="<%=path%>/requirement/editFrequency?id=" + id;
}

function addNext(){
	 var url = "${webRoot}/rcode/saveInfoFrequency?name="+$("#name").val();
	 $.post(url,function(data){
		if(data=="01"){
			window.location.href="<%=path%>/rcode/listContentFrequency";
		}else{
...
function update(i){
	var id1 = $("#id_"+i).attr("value");
	 var url = "${webRoot}/rcode/updateInfoFrequency?id="+id1+"&name="+$("#name").val();
	 $.post(url,function(data){
		if(data=="01"){
			window.location.href="<%=path%>/rcode/listContentFrequency";
		}else{
...
function openDelModel(id){
	...
	var url = "${webRoot}/rcode/deletebyidFrequency?id="+id1;
	$.post(url,function(data){
		if(data=="1"){
			window.location.href="<%=path%>/rcode/listContentFrequency";
		} else {
