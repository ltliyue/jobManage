...
function ruleselect(){
	...
	$.post(url,function(data){
		...
		$.each(json,function(i) {
			if(i==0){
				$("#bm").html('${bm}'+pinyin.getCamelChars(json[0].name));
			}
			op += "<option value='"+json[i].id+";"+pinyin.getCamelChars(json[i].name)+"'>"+json[i].name+"</option>";
		});
	$("#ruleselect").html(op);
...
function hivec(v){
	if(v=='s'){
		...
		var url = "<%=path%>/datasource/hiveshows?bm=#";
		$.post(url,function(data){
			json = eval('(' + data + ')');
			var y = json.ybiao;			
			if(y==null){
	...
	if(v=='h'){
		...
		var url = "<%=path%>/datasource/imports?n="+$("#bmoc").val()+"&id=${taskid}"+"&where="+$("#wbiao1").val();
		$.post(url,function(data){
			json = eval('(' + data + ')');
			alert(json);
	...
	if(v=='p'){
		var url = "<%=path%>/datasource/catim?id=${taskid}";
		$.post(url,function(data){
			json = eval('(' + data + ')');
	...
	function cx(){
		...
		if(task_url!=null&task_url!=""){
			where+="tasku="+task_url+";";
		}
		$("#zdydiv input").each(function(){
		...
		if(where!=""){
			...
			$("#where").val(where);
			var url = "<%=path%>/datasource/listd?id=${taskid}&bm="+$("#obm").val()+"&yeshu=1";
			var datas = {'where':where};
			$.post(url,datas,function(data){
				json = eval('('+data+')');
				$("#th").html(json.th);
	...
	function cqcheck(){
		 if($("input[name='ischeck']:checked").length>0){
		  		if(confirm("确定要去重？")){
		  			...
		  			var url = "<%=path%>/datasource/dis?bm="+$("#obm").val()+"&where="+where+"&taskid=${taskid}&instanceid="+$("#chinst").text()+"&ruleid="+$("#ruleid").val();
		  			$.post(url,function(data){
		  				alert("去重了 "+data+" 条数据!");
	...
	function plup(){
		if(confirm("确定要修改？")){
			...
			var url = "<%=path%>/datasource/plup?bm="+$("#obm").val()+"&col="+$("#tjsel2").val()+"&set="+$("#xghz").val()+"&taskid=${taskid}&instanceid="+$("#chinst1").text()+"&ruleid="+$("#ruleid").val();
	...
	function pldel(){
		if(confirm("确定要删除？")){
			...
			var url = "<%=path%>/datasource/pldel?bm="+$("#obm").val()+"&taskid=${taskid}&instanceid="+$("#chinst2").text()+"&ruleid="+$("#ruleid").val();
			var datas = {"where":where};
			if(where==''){
				if(confirm("确定要全部删除？")){
					$.post(url,datas,function(data){
	...
	function tjinst(){
		...
		$("#chinst1").text(r);
		$("#chinst2").text(r);
		$("#wbiao1").val(" where instance_id = '"+r+"'");
		$("#close7").click();
	}
</script>
...
<a href="#" class='renwu_select'  data-toggle="modal" id="ddata1" hidden="hidden"  data-target="#modifyInstance2">数据筛选
</a>
<a href="#" class='renwu_select'  data-toggle="modal" id="ddata2" hidden="hidden"  data-target="#modifyInstance3"><span class="glyphicon glyphicon-minus" aria-hidden="true"></span>
<span class="glyphicon-class">数据去重</span>
</a> 
<a href="#" class='renwu_select' data-toggle="modal" id="ddata3" hidden="hidden"  data-target="#modifyInstance4">批量修改
</a>
<a href="#" class='renwu_select' data-toggle="modal" id="ddata4" hidden="hidden"  data-target="#modifyInstance5">批量删除</a>
<a href="#" class='renwu_select' id="ddata5" hidden="hidden"  onclick="excelexport()">Excel导出</a>
<a href="#" class='renwu_select' id="ddata6" data-toggle="modal" hidden="hidden"  data-target="#modifyInstance6">SQL窗口</a>
<a href="#" class='renwu_select' id="ddata7" data-toggle="modal" hidden="hidden"  data-target="#modifyInstance8">导入主版本库</a>
</div>
...
<div class="modal hide fade" id="modifyInstance2" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div  class="modal-body">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabel2">数据筛选</h4>
      </div>
      ...
      <div class="modal-header">
                ...
                <div class="input-group date form_date col-md-5" data-date="" data-date-format="" data-link-field="dtp_input2" data-link-format="yyyy-mm-dd">
                    <input class="form-control" size="6" type="text" id="time_start" value="" readonly placeholder="抽取时间起">
                    <span class="input-group-addon"></span>
					<span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
                </div>
                <br>
   				 <div class="input-group date form_date col-md-5" data-date="" data-date-format="" data-link-field="dtp_input2" data-link-format="yyyy-mm-dd">
                    <input class="form-control" size="6" id="time_end" type="text" value="" readonly placeholder="抽取时间止">
                    <span class="input-group-addon"></span>
					<span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
                </div>
      </div>
      <div class="modal-header" >
			<div class="form-group" >
				<span >任务URL:</span>
				<input class="form-control" id="task_url"  type="text" value="" placeholder="任务URL">
			</div>
      </div>
       <div class="modal-header" >
			<div class="form-group" >
				自定义增加筛选条件：<select class="selectpicker" data-style="" data-hide-disabled="true"  id="tjsel">
				</select>
				<a href="javascript:void(0)" onclick="selectadd()"><i class="icon-plus"></i>
				</a>
			</div><br>
			<div class="form-group" id="zdydiv">
...
<div class="modal hide fade" id="modifyInstance3" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-header" >
      <span>请先选择去重的实例ID:</span><span id="chinst"></span><br>
      <a href="#" data-toggle="modal"    data-target="#modifyInstance7" onclick="seleinstance()">选择实例ID</a>
...
<div class="modal hide fade" id="modifyInstance4" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body" >
      <span>请先选择去重的实例ID:</span><span id="chinst1"></span><br>
      <a href="#" data-toggle="modal"    data-target="#modifyInstance7" onclick="seleinstance()">选择实例ID</a>
      <div class="form-group" >
...
<div class="modal hide fade" id="modifyInstance5" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-body">
      ...
      <div  class="modal-header">
    	<span>请先选择去重的实例ID:</span><span id="chinst2"></span><br>
      <a href="#" data-toggle="modal"    data-target="#modifyInstance7" onclick="seleinstance()">选择实例ID</a>
     <div class="form-group" >
...
<div class="modal hide fade" id="modifyInstance6" role="dialog" aria-labelledby="myModalLabel">
 ...
 <!-- Modal 导入数据-->
<div class=" modal hide fade" style="width: 650px" id="modifyInstance8" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" >
    <div class="modal-content">
      ...
      <div class="modal-header" >
			<div class="form-group">
				<label for="exampleInputEmail1">1.导入条件:</label>
				<div>hbase表：global_main_version</div>
				<div>请输入sql语句的where条件：
				<a href="#" data-toggle="modal"    data-target="#modifyInstance7" onclick="seleinstance()">选择实例ID</a>
				<textarea style="width: 95%" rows='3' id="wbiao1"  placeholder="">where instance_id = 请选择实例ID </textarea></div>
				<button class="btn btn-small" onclick="hivec('h')"  id="h">点击导入</button >
			</div>
...
<!-- Modal 筛选数据-->
<div class="modal hide fade" id="modifyInstance7" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div  class="modal-body">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
	...
	<div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary"  id="button" onclick="tjinst()">提交</button>
      </div>		
