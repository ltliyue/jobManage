...
outline-style: dotted;
	...
	function see(id){
		window.location.href="<%=path%>/datasource/detailclean?id="+id; 
	};
	...
	function sjjf(){
		...
		if(r!=null){
			var url = "<%=path%>/datasource/handleLook?instanceId="+r;
			$.post(url,function(data){
				json = eval('('+data+')');
					if(json=="2"){
						if(confirm("已经完成，是否撤回？")){
							var url1 = "<%=path%>/datasource/handleMon?instanceId="+r+"&s=0";
						...
						if(confirm("清洗是否完成？")){
							var url2 = "<%=path%>/datasource/handleMon?instanceId="+r+"&s=3";
...
<h2 class="content_title">${title }</h2>
         ...
         <div class="span9" style="width:78%;">
            <table class="table  table-striped content_rwgl">
              ...
              <form action="<%=path%>/dataclear/dataclean" id="form1">
					...
					<c:if test="${not empty tasks}">
						<c:forEach var="task" items="${tasks }" varStatus="ts">
							<tr align="left" style="cursor: pointer;">
								...
								<a href="#" onclick="complete(${task.taskId })" class="content_xuqiumcan" data-toggle="modal"    data-target="#modifyInstance7">完成</a>
								</td>
							</tr>
						</c:forEach>
					</c:if>
...
<div class="modal hide fade" id="modifyInstance7" role="dialog" aria-labelledby="myModalLabel">
 <div class="modal-dialog" role="document">
	...
	<div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary"  id="button" onclick="sjjf()">完成</button>
      </div>		
 </div>
</div>
