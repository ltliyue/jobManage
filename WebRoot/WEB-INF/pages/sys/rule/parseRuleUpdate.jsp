...
<script type="text/javascript">
	...
	$(document).ready(function() {
		var url = '${webRoot}/parseRule/getParseRule?id=${id}';
		$.post(url, function(data) {
			...
			$("#loopSample").val(json.loopSample);
			$("#loopStart").val(json.loopStart);
			$("#loopEnd").val(json.loopEnd);
			$("#loopVar").val(json.loopVar);
			$("#loopStep").val(json.loopStep);
			$("#loopFormat").val(json.loopFormat);
		...
		var url = '${webRoot}/parseRuleMidMap/getMidMapByRule?id=${id}';
		$.post(url, function(data) {
			json = eval('(' + data + ')');
	...
	function submit() {
		...
		var ischecks = $("input[name='check']:checked").length;
		var url = '${webRoot}/parseRule/updateParseRule';
							...
							if (data == "1") {
								...
								for (var i = 1; i < tableObj.rows.length; i++) { //遍历Table的所有Row
									...
									rowElement.cells[1].firstChild
											.setAttribute("name", "varName");
									rowElement.cells[2].firstChild
											.setAttribute("name", "datatype");
									if (rowElement.children[0]
										...
										var url = '${webRoot}/parseRuleMidMap/updateParseRuleMidMap';
										data1 = $("#formn").serialize();
										$.post(url, data1, function(data) {
											if (data != "" && data == "0") {
												midMapModifySuc = false;
	...
	function delMidMap(row) {
		...
		//获取删除的行的id
		var idInput = tr.firstChild;
		var id = idInput.value;
		//弹出提示框
		if (confirm("确定删除该映射吗吗？")) {
			tr.parentNode.removeChild(tr);
			var url = "${path}/crawlManage/parseRuleMidMap/deleteParseRuleMidMap?id="
					+ id;
			$.post(url, function(data) {
				alert("删除完成！");
...
<body style="margin: 0; padding: 0">
	...
	<div class="content_waikuang">
		<div class="container-fluid search_select select-result"
			...
			<h2 class=".content_title">URL解析规则配置修改</h2>
			<form action="${webRoot}/parseRule/updateParseRule" id='formn'>
				...
				<div class="form-group">
					<input type="checkbox" id="urlWithLoopCheck"
						...
						type="hidden" id="urlWithLoop" name="urlWithLoop" value="1"></input>
				</div>
				<br>
				...
				<div class="form-group" id="loopVarDiv" style="display: none;">
					...
					<input type="text" class="form-control content_zjnrcqtext"
						name="loopVar" id="loopVar"
						placeholder="循环变量名或改变的数字的位置，若为$，则表示文件名的最后，变量需要使用{{变量名}}">
				</div>
				...
				<div class="form-group" id="loopFormatDiv" style="display: none;">
					...
					<input type="text" class="form-control content_zjnrcqtext"
						name="loopFormat" id="loopFormat" value="%d">
				</div>
				<div class="form-group">
					<label for="urlType" class="content_zjnrcqlabel">生成的URL的类型</label>
					<select class="form-control" name='urlType' id="urlType"
						onchange="choseExtractType(this.value)" datatype="n"
						nullmsg="请选择生成的URL的类型" errormsg="必须选择生成的URL的类型">
				...
				<div class="form-group" id="testUrlDiv">
					<label for="testUrl">测试url</label><br> <input type="text"
						class="form-control content_zjnrcqtext" name='testUrl'
						id="testUrl" style="width: 70%; float: left" placeholder="测试url" /><br>
					<br>
				</div>
			</form>
