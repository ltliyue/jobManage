...
%>
<!-- 
	注意事项： 1.在使用自定义的方式时，上传的jar包中必须包含源文件
		   2.读取jar包中的java文件判断字段名，插入规则
 -->
<!DOCTYPE html>
<html lang="zh-CN">
<head>
...
<script type="text/javascript"
	src="<%=path%>/resources/js/treegrid/sljquery.treegrid.js"></script>
<script type="text/javascript"
...
<script type="text/javascript">
	...
	$(document).ready(function() { 
		if($("#taskId").val()!=null && $("#taskId").val()!=""){
			$("#taskId").attr("readonly","readyonly");
		}
		...
		$('#modifyInstance').on(
				...
				function(event) {
					...
					modal.find('#siteItemChooseIFrame').attr(
							"src",
							"${webRoot}/extractRule/siteItemChoose?taskId=${taskId}&eventOwner="
									+ id);
	...
	<div class="content_waikuang">
		<div class="container-fluid search_select select-result"
			...
			<form action="${webRoot}/datauser/useradd" id='formn'>
				<input type="text" style="display:none" name="modal" id="modal" value="${modal}">
				...
				<div>
					<label for="taskId" class="content_zjnrcqlabel">任务ID</label> <input
						...
						errormsg="任务Id必须为字符串,不能包含标点符号" value="${taskId}" onchange="initStoreColumn()">
				</div>
				...
				<div>
					...
					<!-- 
						<a onclick="selfDef()" style="color: #08c;text-decoration: underline;background:none">自定义模式</a>
					 -->
				</div>
						...
						<table class="table table-striped" id="elementExtractRules0">
							...
							<tbody id="elementExtractRulesTbody0">
								<tr>
									...
									<td>
										<select class="from-control" id="storeColumnName0"  name="storeColumnSet" onchange="setStoreColumnComment(this)">
										</select>
				...
				<div id="testUrlDiv">
					<label for="testUrl" class="content_zjnrcqlabel">测试url</label> <input
						...
						id="testUrl" placeholder="测试url"  datatype="*" nullmsg="请输入测试url，必须进行测试"
