...
function checkDetail(id) {
	window.location.href="<%=path%>/requirement/editStatus?id=" + id;
}

function addNext(){
	 var url = "${webRoot}/rcode/saveInfoStatus?name="+$("#name").val();
	 $.post(url,function(data){
		if(data=="01"){
			window.location.href="<%=path%>/rcode/listContentStatus";
		}else{
...
function update(i){
	var id1 = $("#id_"+i).attr("value");
	 var url = "${webRoot}/rcode/updateInfoStatus?id="+id1+"&name="+$("#name").val();
	 $.post(url,function(data){
		if(data=="01"){
			window.location.href="<%=path%>/rcode/listContentStatus";
		}else{
...
function openDelModel(id){
	...
	var url = "${webRoot}/rcode/deletebyidStatus?id="+id1;
	$.post(url,function(data){
		if(data=="1"){
			window.location.href="<%=path%>/rcode/listContentStatus";
		} else {
