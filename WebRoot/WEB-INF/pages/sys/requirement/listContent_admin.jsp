...
function excelexport(){
	if(confirm("确定要导出？")){
		var url = "<%=path%>/requirement/export?&bm=需求评估";
					...
					<table style="clear: both">
						...
						<tr>
							...
							<td>提出时间 <input class="laydate-icon"
								...
								name='requireMent.proposeTimeF' id="proposeTime"
								style="width: 150px" nullmsg="请输入提出时间！"> &nbsp-&nbsp <input
								class="laydate-icon"
								onclick="laydate({istime: true, format: 'YYYY-MM-DD'})"
								name='requireMent.proposeTimeT' id="proposeTime"
								style="width: 150px" nullmsg="请输入提出时间！"></td>
						</tr>
						<tr>
							<td>要求时间 <input class="laydate-icon"
								onclick="laydate({istime: true, format: 'YYYY-MM-DD'})"
								name='requireMent.requestTimeF' id="requestTime"
								style="width: 120px" nullmsg="请输入要求时间！"> &nbsp-&nbsp <input
								class="laydate-icon"
								onclick="laydate({istime: true, format: 'YYYY-MM-DD'})"
								name='requireMent.requestTimeT' id="requestTime"
								style="width: 120px" nullmsg="请输入要求时间！"></td>
							<td></td>
									...
									<tr>
										...
										<c:if test="${account.status  =='待审核'}">
											<td><a href="javascript:"
												onclick="access('${account.id }','${account.name }','${account.type }');">通过</a>
											</td>
											<td><a href="javascript:"
...
function access(id,name,type) {//审核
	if (confirm("确定审核通过名称为："+name+"  的需求吗？")) {
		var url = "<%=path%>/requirement/updateInfo?id="+id+"&status=已审核&reason=审核通过&type="+type;
