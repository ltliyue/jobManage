...
public class RCodeController extends BaseController {
	 ...
	 * @throws UnsupportedEncodingException
	 */
	@RequestMapping(value = "/listContentGroup", method = RequestMethod.GET)
	public ModelAndView contentGroup(RCode record) throws Exception {

		System.out.println("--listContentGroup--需求查询条件：" + record.toString());

		List<RCode> list = listContentGroupq(record);

		System.out.println("--listContentGroup--需求查询结果：" + list);

		Map<String, Object> map = new HashMap<String, Object>();
		map.put("accounts", list);
		map.put("page", record);
		return request("sys/rcode/listContentGroup", map);
	}
	 ...
	 * @param name
	 */
	@RequestMapping(value = "/listContentGroupq", method = RequestMethod.POST)
	@ResponseBody
	public List<RCode> listContentGroupq(RCode record) throws UnsupportedEncodingException {

		List<RCode> list = service.getResourceAllGroup(record);

		return list;
	}

	@RequestMapping(value = "/saveInfoGroup", method = RequestMethod.POST)
	@ResponseBody
	public void saveInfoGroup(String id, String name, PrintWriter out) throws UnsupportedEncodingException {
		RCode query = new RCode();
		query.setId(Integer.parseInt(TimeCompare.turnTime3(new Date())));
		query.setName(name);

		System.out.println("----saveInfoGroup--查询条件：" + query.toString());

		int result = service.saveInfoGroup(query);
		System.out.println("----saveInfoGroup--保存结果：" + result);
		if (result == 1) {
			System.out.println("----信息保存成功");
			out.print("01");
		} else {
	 ...
	 * @param name
	 */
	@RequestMapping(value = "/updateInfoGroup", method = RequestMethod.POST)
	@ResponseBody
	public void updateInfoGroup(String id, String name, PrintWriter out) throws UnsupportedEncodingException {
		RCode query = new RCode();
		query.setId(Integer.parseInt(id));
		query.setName(name);
		...
		int result = service.updateInfoGroup(query);
		System.out.println("----update保存结果：" + result);
		if (result == 1) {
			System.out.println("----update修改成功");
			out.print("01");
	 ...
	 * 删除任务 id
	 */
	@RequestMapping(value = "/deletebyidGroup", method = RequestMethod.POST)
	@ResponseBody
	public int deletebyidGroup(Integer id) {
		System.out.println("删除：" + id);
		return service.deleteByPrimaryKeyGroup(id);
	}
}
