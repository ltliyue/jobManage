...
public class CacheOperation {
   ...
   private void addCacheData(String key, Object data, boolean check) {
       if (Runtime.getRuntime().freeMemory() < 5L*1024L*1024L) {//虚拟机内存小于10兆，则清除缓存
    	   System.out.print("WEB缓存：内存不足，开始清空缓存！");
           removeAllCacheData();
           return;
       } else if(check && cacheMap.containsKey(key)) {
    	   System.out.print("WEB缓存：key值= " + key + " 在缓存中重复, 本次不缓存！");
           return;
       }
...
//参数类型是MethodInfo则调用该方法的返回值做这参数
           ...
           System.out.print("调用方法失败,methodName=" + methodName);
           if (key != null) {
               removeCacheData(key);
               System.out.print("更新缓存失败，缓存key=" + key);
           }
           e.printStackTrace();
       }
       return returnValue;
